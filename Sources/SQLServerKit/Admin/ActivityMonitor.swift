...
public struct ActivityMonitor: Sendable {
    ...
    private static let benignWaitTypes = [
        ...
        "SOS_WORK_DISPATCHER", "DISPATCHER_QUEUE_SEMAPHORE", "LOGMGR_QUEUE",
        "CHECKPOINT_QUEUE", "CHKPT", "HADR_FILESTREAM_IOMGR_IOCOMPLETION",
        "HADR_WORK_QUEUE", "HADR_TIMER_TASK", "HADR_CLUSAPI_CALL", "HADR_LOGCAPTURE_WAIT",
        "HADR_NOTIFICATION_DEQUEUE", "FT_IFTS_SCHEDULER_IDLE_WAIT", "FT_IFTSHC_MUTEX",
        ...
        "WAIT_FOR_RESULTS", "WAIT_XTP_HOST_WAIT", "WAIT_XTP_OFFLINE_CKPT_NEW_LOG",
        "WAIT_XTP_CKPT_CLOSE", "WAIT_XTP_RECOVERY", "XE_DISPATCHER_JOIN",
        "XE_DISPATCHER_WAIT", "XE_TIMER_EVENT", "PARALLEL_REDO_WORKER_WAIT_WORK",
        "PARALLEL_REDO_DRAIN_WORKER", "PARALLEL_REDO_LOG_CACHE",
        "PARALLEL_REDO_TRAN_LIST", "PARALLEL_REDO_WORKER_SYNC",
        "PREEMPTIVE_XE_GETTARGETSTATE", "PREEMPTIVE_OS_FLUSHFILEBUFFERS",
        "PVS_PREALLOCATE", "VDI_CLIENT_OTHER", "DBMIRROR_DBM_EVENT",
        "DBMIRROR_EVENTS_QUEUE", "DBMIRROR_WORKER_QUEUE", "DBMIRRORING_CMD",
        "SLEEP_TASK", "SLEEP_SYSTEMTASK", "STARTUP_DEPENDENCY_MANAGER",
        "SERVER_IDLE_CHECK", "RESOURCE_QUEUE", "BROKER_TO_FLUSH",
        "MEMORY_ALLOCATION_EXT", "SNI_HTTP_ACCEPT", "SOS_SCHEDULER_YIELD"
    ]
    private static let benignWaitPrefixes = [
        "CLR_", "SLEEP_", "BROKER_", "XE_", "SQLTRACE_", "PREEMPTIVE_", "QDS_", "HADR_",
        "FT_", "DBMIRROR", "PARALLEL_REDO_", "PWAIT_", "WAIT_XTP_"
    ]
